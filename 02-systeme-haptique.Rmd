...
| --- | --- | --- |
| **les extérocepteurs :**
 nous renseignent sur le monde extérieur | **télérécepteurs :**
 perception à distance | récepteurs visuels |
| récepteurs auditifs |
| récepteurs olfactifs |
| **récepteurs de
 contact** | récepteurs gustatifs |
| récepteurs cutanés du toucher |
| **les
 intérocepteurs :** nous renseignent sur notre propre organisme | **viscérocepteurs :**
 disséminés dans les viscères | nous renseignent sur un paramètre
 physiologique interne (la douleur par exemple) |
| **propriocepteurs :**
 regroupés (oreille interne) ou disséminés (fuseaux
 neuro-musculaires) | nous renseignent sur un paramètre lié
...
1. le sens statique ou vestibulaire : sens qui a
...
* Le toucher orienté: une stimulation de la peau avec
 une stimulation vestibulaire [1](#note2).
...
1. [du système perceptif lié au toucher et à la
 kinesthésie](007-le-systeme-haptique-cote-perception-la-somesthesie.html),
1. [et des mouvements d’exploration](008-le-systeme-haptique-cote-action-le-systeme-moteur.html).
...
Les récepteurs physiologiques de la perception tactile se trouvent dans la
 peau (figure [1.1](#cap:Structure-de-la)). La bande passante
...
Figure 1.1 : Structure de la
 peau (emprunté à ([Reznik,
 1997](047-bibliographie.html#Reznik1997)))
...
* les thermorécepteurs : liés aux sensations de chaud
 et de froid ;
* les nocirécepteurs : liés à la sensation de
 douleur ;
* les mécanorécepteurs : liés à la discrimination
 tactile.


On distingue 4 types de mécanorécepteurs [@cholewiak2013sensory], [@kalawsky1993science], [@seow1988physiology] ; Il sont présentés dans le
 tableau [1.2](#cap:tactile).
...
Table 1.2 : Principaux récepteurs tactiles [@burdea1996force]
...
La perception sensitive donnée au niveau de la peau peut être ainsi
 représentée grâce à un homonculus (figure [1.2](#cap:hom)).



![](img/homonculus_sensitif.png)
![](img/homonculus.png)
Figure 1.2 : Homoculi sensitifs humains : à gauche la vue est interne
 et calquée sur une coupe du cerveau ; à droite, les dimensions des
 membres et du corps ont été exagérées selon les capacités sensoriels
...
La perception des signaux spatiaux dépend de la fréquence du signal. La
 ...
 kinesthésiques et tactiles. La figure [1.3](#cap:Le-spectre-de) illustre la notion de perception haptique, en
 terme de fréquence des stimuli.



![](img/hapticSpectrum.png)
Figure 1.3 : Le spectre de la perception haptique.
 [@wall2004investigation]
...
## Le système haptique côté action : le système moteur


Nous allons maintenant présenter l’autre côté du système haptique tel que
 nous le considérons. La partie [1.2](007-le-systeme-haptique-cote-perception-la-somesthesie.html#sub:Le-syst=E8me-haptique) concernait le sous-système
...
1. du système perceptif lié au toucher et à la
 kinesthésie (vu en [1.2](007-le-systeme-haptique-cote-perception-la-somesthesie.html#sub:Le-syst=E8me-haptique)),
...
De manière similaire à ce que l’on a pour la perception tactile, il a été
 ...
 (figure [1.4](#cap:hom2)mot}).



![](img/homonculus_moteur.png)
...
\begin{equation}
 MT=a+b\log\_{2}(2A/W)\label{eq:fitts}\end{equation}
...
La figure [1.5](#cap:L'exp=E9rience-de-Fitts) illustre
...
Enfin, on peut citer quelques extensions à la loi de Fitts. En effet,
 ...
 effectué en deux parties (figure [1.6](#cap:T=E2che-pour-n=3D3D2)),
...
et pour n parties (figure [1.7](#cap:T=E2che-pour-n)):
...
La loi d’Accot ou *steering law* [@accot1997beyond] est beaucoup plus récente, et
 ...
 déroulant à plusieurs niveaux (figure [1.8](#cap:Le-chemin-emprunt=E9)).



![](img/menu_accot.png)
Figure 1.8 : Le chemin emprunté pour aller chercher
...
\begin{equation}
 T\_{c}=a+b\int\_{C}\frac{ds}{W(s)}\label{eq:accot}\end{equation}
...
Des chemins plus simples permettent des simplifications mathématiques. Par
 exemple, si le chemin est un tunnel droit avec une largeur *W* constante, l’équation \eqref{eq:accot} devient :
...
Après un calcul de variation, il s’ensuit que la trajectoire minimisant le
 jerk et allant d’un point *x*=*x**i* à un point
 *x*=*x**f* en *t*=*d*sec aura pour équation :

\begin{equation}
 x(t)=x\_{i}+\left(x\_{f}-x\_{i}\right)\left(10\left(\frac{t}{d}\right)^{3}-15\left(\frac{t}{d}\right)^{4}+6\left(\frac{t}{d}\right)^{5}\right)\label{eq:jerk}
 \end{equation}
...
\[ x(t)=800t³-2400t^{4}+1920t^{5},\quad0\leq t\leq0,5sec\]
 ...
 La figure [1.9](#cap:Position,-vitesse,-acc=E9l=E9ration)
...
\begin{equation} \underline{x}(t)=\left[\begin{array}{cc} x(t)
 ...
 y(t) &
 =y\_{i}+\left(y\_{f}-y\_{i}\right)\left(10\left(\frac{t}{d}\right)^{3}-15\left(\frac{t}{d}\right)^{4}+6\left(\frac{t}{d}\right)^{5}\right)\end{array}\right]\label{eq:jerk2}\end{equation}
...
![](img/meyer.png)
Figure 1.10 : modèle d’impulsion
 initiale optimisée ([Meyer et al.,
 1988](047-bibliographie.html#Meyer1988))
...
[1](#text3)
même si l’on ne doit pas, ceci restant une
...
## L’exploration haptique : quand l’action et la perception se conjuguent sur le canal haptique
...
La figure [1.11](#cap:actionHaptique) illustre les boucles
...
![](img/bouclesPerception.png)
Figure 1.11 : Schéma de la
 perception/action haptique (d’après [@wall2004investigation])
...
[@dennerlein2000force] ont proposé
 ...
 (figure [1.12](#cap:Les-diff=E9rentes-g=E9om=E9tries)).



![](img/manipulation.png)
Figure 1.12 : Les différentes géométries
 des prises de la main
