...
Un système interactif est généralement composé de deux parties :
 l’interface utilisateur et le noyau fonctionnel (\@ref(fig:decompositiondunsysteme)). L’interface utilisateur est
...
![(#fig:decompositiondunsysteme) Décomposition d’un système interactif](img/sys_inter.png)
...
Il est possible de modéliser un système interactif suivant plusieurs
 ...
 jeu dans l’utilisation du système. La figure [2.2](#cap:mod=E9lisations-d'un-syst=E8me) reflète différentes approches
 que l’on peut avoir d’un système



![](img/I21.png)
Figure 2.2 : Modélisations d’un système
 informatique interactif. Les flèches indiquent les relations de
 dépendance de chaque modèle (tiré de [@baudel1995aspects])
...
Dans leur modèle, The Model Human Processor , S. Card, T. Moran et A.
 ...
 paramètres (figure [2.3](#cap:Le-mod=E8le-du)).
...
![](img/humanProcessor.png)
Figure 2.3 : Le modèle du processeur humain (d’après
 [@booth2004human])
...
![](img/systemecognitif.png)
Figure 2.4 : Le système sensoriel visuel
 et sa relation avec le système cognitif (tiré et adapté de [@coutaz1988interface])
...
* *la* *main dominante, constituée des 5
 ...
 l’interaction.*[...]([Sturman,
 1992](047-bibliographie.html#Sturman1992))
...
[@norman1988psychology] est une approche cognitiviste de la modélisation de l’utilisateur
 ...
 interne que l’utilisateur se fait du système. La figure [2.5](#cap:norman) illustre ce modèle de l’action. Le point fort de
...
Pour manipuler un point dans l’espace virtuel de la mémoire de
 ...
 commun de ces dispositifs, c’est la souris. La figure [2.6](#cap:La-premi=E8re-souris) montre la première souris de l’histoire,
...
La figure cap :Espace-de-conception reprend la taxonomie de [@card1991morphological], ainsi que celle de chacun
...
![](img/cards.png)
Figure 2.7 : Espace
 de conception des dispositifs d’entrée (tiré de [@card1991morphological])
...
Pour évaluer les performances des dispositifs de pointage, les recherches
 ont souvent utilisé la loi de Fitts (voir par :Loi-de-Fitts). La
 figure cap :Les-performances-des montre les performances de différents
...
![](img/steering.png)
Figure 2.8 : Les
 performances des périphériques de pointage selon la loi d’Accot (tiré
 de [@accot1999performance])
...
Une autre étude ([MacKenzie et al.,
 2001](047-bibliographie.html#MacKenzie2001)) confirme ce résultat (figure [2.9](#cap:Les-performances-de)).



![](img/comparaison.png)
Figure 2.9 : Les
 performances de dispositifs de pointage (adapté de [@mackenzie2001accuracy]
...
Les sujets devaient aller pointer le plus vite possible, différentes
 cibles placées sur un cercle. La figure [2.10](#cap:La-t=E2che-de) précise l’ordre des mouvements à réaliser.



![](img/CHI01-f6.jpg)
Figure 2.10 : La tâche de pointage dans l’expérience de
 McKenzie



Sur le graphique de la figure [2.9](#cap:Les-performances-de), on retrouve bien le fait que la souris
 est bien plus efficace que les autres dispositifs de pointage. Il y a
 également un effet d’apprentissage qui apparaît (le temps du mouvement
 diminue au fur et à mesure de l’expérience), mais ce n’est pas le sujet de
 notre discussion.


Enfin, d’un point de vue ergonomique, [(Zhai
 et al., 1996)](047-bibliographie.html#Zhai1996) a démontré que les périphériques d’entrée manipulés
 avec les doigts, obtiennent de meilleures performances lors d’une tâche de
 pointage, que les dispositifs qui nécessitent la mise en action du poignet
 et/ou du coude et/ou de l’épaule.
...
Dans un éditeur d’objets en deux dimensions, [@kurtenbach1991issues] ont utilisé
 ...
 figure [2.11](#cap:Les-gestes-pour) présente ces gestes. Par
...
![](img/BarfieldF16.png)
Figure 2.11 : Les gestes pour
 l’édition graphique. (tiré de [@kurtenbach1991issues])
...
Pour les artistes, l’entrée gestuelle peut faciliter l’interaction
 ...
 sur la figure , les formes des notes les plus communes (en haut de la
...
![](img/BarfieldF17.png)
Figure 2.12 : Les gestes pour transcrire
 la notation musicale (tiré de [@buxton1995chunking])
...
La télé-robotique est le domaine technologique qui a nécessité la création
 ...
 Maître-esclave . Sheridan ([Sheridan,
 1992](047-bibliographie.html#Sheridan1992)) a défini le système télé-opérateur Maître-esclave comme
 suit :
...
Nous pouvons considérer que le premier périphérique à retour de force
 ...
 l’Argonne (voir la figure [2.13](#cap:L'argonne)), un système
...
Par la suite, d’autres périphériques ont été conçus pour la
 ...
 pu atteindre leur but initial (voir figure [4.4](027-l-interaction-non-visuelle.html#cap:dufrene)), grâce à la montée en puissance des
 ordinateurs ([Brooks et al.,
 1990](047-bibliographie.html#Brooks1990)).



![](img/molecular.png)
Figure 2.14 : Molecular Docking Virtual
 Interface ou MDVI (University of North Carolina)
...
Enfin, vers la fin des années 70, est apparu le premier prototype de
 ...
 Sandpaper system développé au MIT ([Minsky,
 1995](047-bibliographie.html#Minsky1995)). Il s’agit d’un joystick offrant 2 degrés de liberté et rendant à
...
Nous pouvons citer le Rutgers Master II qui est basé sur les travaux de
 ...
 de fermer totalement la main (figure [2.15](#cap:Les-gants-Rutgers)).



![](img/rutgers.png)
Figure 2.15 : Les gants
 Rutgers Master II et CyberGrasp
...
Les bras maîtres sont principalement utilisés dans les applications de
 télé-opérations. Sur la figure [2.16](#cap:Le-SARCOS-Dextros)
...
![](img/dextroslarge.png)
Figure 2.16 : Le SARCOS
 Dextrous Arm Master
...
Ce sont des périphériques proposant au minimum trois degrés de liberté en
 ...
 parfois six. La figure [2.17](#cap:Les-degr=E9s-de) illustre
 les différentes possibilités en terme de degrés de liberté.



![](img/DoF.png)
Figure 2.17 : Les degrés de liberté sur un bras à retour
 de force



Le périphérique le plus utilisé dans les laboratoires est le PHANTOM
 (figure [2.18](#cap:le-PHANTOM)), créé et commercialisé par
 Sensable Inc.



![](img/phantom-omni.png)
...
Nous pouvons citer le Virtuose 3D, le Delta Haptic et le Freedom 6S
 (figure [2.19](#cap:Le-Virtuose-3D,)), également utilisés
...
![](img/delta_6dof.jpg)
![](img/freedom6s.png)
Figure 2.19 : Le Virtuose 3D, le Delta Haptic et le
 Freedom 6S
...
Les souris à retour de force sont des dispositifs à deux degrés de
 ...
 (figure [2.20](#cap:mouse)), conçue par Immersion, et
...
Les dispositifs tactiles sont bien entendu basés sur [les perceptions
 tactiles](007-le-systeme-haptique-cote-perception-la-somesthesie.html#toc3). Nous pouvons par exemple rappeler, que la perception
...
Pour le reste, donc, les dispositifs sont surtout axés sur la
 ...
 tactile est celle visible sur la figure [2.21](#cap:Une-cellule-d'afficheur). On peut y voir une cellule
 d’affichage tactile. Cette cellule est composée de petits picots capables de
 monter ou descendre sur leur axe. Il ainsi possible de dessiner des petits
 motifs en relief.



![](img/braille_cell3.jpg)
Figure 2.21 : Une cellule d’afficheur
 tactile



Les afficheurs brailles (figure [2.22](#cap:Une-plage-braille)) utilisent ces cellules. Comme un caractère
...
![](img/braille-1.jpg)
Figure 2.22 : Une plage
 braille



Toujours dans la même approche, les sociétés ABTIM [1](#note4) et KGS
 [2](#note5) ont
 équipé de cellules des surfaces plus grande. Il ont ainsi réalisé des
 systèmes capables de reproduire des dessins en relief (figure [2.23](#cap:l'ADVANCED-DISPLAYS-for)).



![](img/tactileDisplay.jpg)
Figure 2.23 : (gauche) : l’ADVANCED DISPLAYS for
 the BLIND de ABTIM  ; (droite) : le DotView de KGS
...
Pour terminer cette partie, nous pouvons évoquer une autre classe de
 ...
 figure [2.24](#cap:Le-couplage-des).



![](img/sourisTactile.jpg)
Figure 2.24 : Le couplage des
 dispositifs de pointage avec le mode tactile
...
D’un point de vue général, il s’avère que les dispositifs actuels
 ...
 PHANTOM [@jansson1999phantom]. De plus,
 la technologie même est un facteur limitant : [@wall2004investigation] a ainsi montré que les moteurs du
 PHANTOM étaient inadéquats pour générer des textures très fines nécessitant
 des hautes fréquences.


En se référant aux [procédures d’exploration haptique](009-l-exploration-haptique-quand-l-action-et-la-perception-se-conjuguent-sur-le-canal-haptique.html#toc11) proposées par
 [@lederman1987haptic], [@wall2004investigation] a dressé le tableau [2.3](#cap:Les-possilibit=E9s-des), qui montre les possibilités et les
 impossibilités, lorsque l’on manipule un dispositif de pointage. Finalement,
 [@lederman2004haptic] ont
 également montré que la rigidité du stylet limitait également la perception
 (par rapport à un stylet flexible).
...
|  |  |
| --- | --- |
| **Mouvements d’exploration** | **Possibilité avec un dispositif de
 pointage** |
|
 Le mouvement latéral (textures) | Possible, bien que les caractéristiques des
 textures varient |
|  | temporellement (vibration) et non pas
 spatialement |
| La pression | Possible |
| Le contact statique | Possible, bien qu’il n’y ait pas de retour
 de température, |
|  | ou de forces distribuées pour générer des
 étirements |
| Le maintient | Possible en attachant l’objet simulé à la
 place de l’activateur |
| L’enveloppement | Non possible en l’absence de plusieurs
 points de contact |
| Le suivi de contours | Possible, mais très difficile du fait d’une
 zone de contact |
|  | réduite à un point |


Table 2.3 : Les possibilités des mouvements
 d’exploration avec un périphérique de pointage
...
Le système haptique humain a un rôle important à jouer dans l’interaction
 ...
 figure [2.25](#cap:Interaction-haptique-entre) illustre la
 double boucle d’interaction liée au système haptique : l’homme et la machine
 agissent et perçoivent sur le mode haptique, en même temps. Les schémas de
 fonctionnement pour l’homme et pour la machine sont très similaires : il y a
 dans les deux cas une boucle action-réaction entre le monde extérieur, et le
 système de décision  ; c’est à l’intersection des deux boucles que se situe
 l’interaction.



![](img/retourhaptic.png)
Figure 2.25 : Interaction haptique entre
 l’homme et la machine (tiré de [@casiez2004contribution])
...
Dans cette partie, nous allons passer en revue quelques applications
 ...
 [cette page](016-les-peripheriques-de-sortie-a-retour-haptique.html#sub:historique) pour un historique
 sur les débuts du retour de force. On rappelle qu’initialement, c’est en
 télé-robotique que les dispositifs à retour de force sont apparus.


#### Le retour haptique comme technique d’interaction


[@miller1999design] ont
 identifié 4 utilisations possibles du retour haptique :


1. *Anticipation* (Anticipation): force
 résistante annonçant l’imminence d’un événement.
1. *Indication* (Suivi) : force donnant
 l’indication qu’une action est en cours.
1. *Follow-through* (Accomplissement): force
 donnant l’indication à l’utilisateur qu’un événement s’est produit.
1. *Guidance* (Guidage): guidage/contrainte du
...
Nous pouvons également citer le H3D API de SenseGraphics[1](#note6).
...
Un logiciel de modélisation tridimensionnelle nommé FreeForm[2](#note7), a été présenté
 par Sensable Technologies Inc. Ce système se base sur une métaphore du
...
De manière similaire, InTouch ([Gregory
 et al., 2000](047-bibliographie.html#Gregory2000)) est un logiciel de modélisation 3D. Il permet
 également le dessin sur un volume (figure [2.26](#cap:Le-logiciel-inTouch)).
...
Le centre lavalois de ressources technologiques [3](#note8) a proposé le système VTT
...
Le retour de force a souvent été utilisé dans les domaines artistiques.
 ...
 instrument réel, comme pour le projet dAb (figure [2.27](#cap:le-syt=E8me-DAB)) : le dispositif utilisé est un PHANTOM, et
 il s’agit d’imiter les sensations que l’on a lors du maniement de
 pinceaux.


Dans une approche très différente, le projet PHASE (Plate-forme Haptique
 d’Application Sonore pour l’Eveil Musical (figure [2.28](#cap:La-plateforme-PHASE))) propose un moyen de création
 complètement nouveau. Le retour haptique est utilisé pour faire sentir les
 éléments d’un monde virtuel que l’on rencontre pendant l’exploration de
 celui-ci.



![](img/dab_system_scheib_baxter-cropped.jpg)
Figure 2.27 : Le système dAb [@bill2001interactive]




![](img/InstalPhaseCGP.jpg)
Figure 2.28 : La
 Plate-forme PHASE ([lambert
 et al., 2005](047-bibliographie.html#lambert2005))
...
Nous allons maintenant nous rappeler qu’à l’origine, le retour de force a
 ...
 les environnements virtuels et télé-robotiques [@rosenberg1993virtual] [@sheridan1992musings]. Aussi, nous allons étudier cet
