...
Nous allons proposer dans ce chapitre un cadre d’utilisation des
 ...
 limites de ce type de dispositif dans le chapitre [2](011-chapitre-2-communications-homme-machine.html).


Maintenant, du potentiel du système haptique (voir ([1](009-l-exploration-haptique-quand-l-action-et-la-perception-se-conjuguent-sur-le-canal-haptique.html#htoc19)), ([2](015-les-peripheriques-d-entree.html#sub:performances)) et ([3](017-les-applications-du-retour-haptique.html#sub:La-perception-via))), nous pouvons déjà édicter plusieurs règles :



**Règle 1 :**
la précision des doigts en terme de positions,
 et de mémorisation ([Zhai et al.,
 1996](047-bibliographie.html#Zhai1996)). Ceci implique l’utilisation des périphériques type PHANTOM en
...
la mémoire sur la position est meilleure que
 celle sur les distances. ([Faineteau
 et al., 2003](047-bibliographie.html#faineteau:03))
**Règle 3 :**
...
De plus, les modèles de l’utilisateur (en particulier [le modèle du
 processeur humain](014-les-modeles-de-l-utilisateur.html#toc15)) nous indiquent quelques règles à respecter sur l’ergonomie des interfaces :
...
Par exemple, sur la figure [5.1](#cap:localisation), un
...
Afin d’illustrer ce concept d’interaction, nous proposons **la métaphore
 du levier de vitesse** (figure [5.2](#cap:La-m=E9taphore-du)), qui comprend nombre de similitudes avec la
...
Il s’agit de la déclaration d’un rectangle dont le coin supérieur gauche
 ...
 rectangle (en l’occurence, en *x*=599 et
 *y*=296).
...
L’effet que nous utilisons pour situer un point haptique est illustré sur
 la figure [5.3](#cap:Illustration-de-l'effet). Il s’agit d’un
...
`![](img/aimant.svg)`
Figure 5.3 : Illustration de l’effet
 utilisé
...
À la lecture du fichier SVG, il est possible de calculer un couple de
 ...
 système de coordonnées du SVG, *x**c*=599, et *y**c*=199. Nous avons
...
Le problème s’est compliqué lorsque nous nous sommes penché sur les
 ...
 une suite de point de la forme "*x*1,*y*1 
 *x*2,*y*2 ...  *x**n*,*y**n*" afin de dessiner
...
La figure [5.4](#cap:Centre-d'intertie-ext=E9rieur)
 illustre cette forme, ainsi que le centre d’inertie calculé : *x**c*=47,14 et
 *y**c*=137,14.



![](img/centre.png)
Figure 5.4 : Centre d’inertie extérieur à
 la forme
...
nous donne la figure [5.5](#cap:Forme-centr=E9e-sur) :
...
Et d’un point de vue logiciel, nous nous sommes appuyé sur l’architecture
 ...
 point par ([Reenskaug, 1979](047-bibliographie.html#Reenskaug1979)), qui
 travaillait alors sur Smalltalk dans les laboratoires de recherche Xerox
 PARC.
...
D’un point de vue technique, le XMLHttpRequest est une fonction javascript
 ...
 pour Asynchronous Javascript And XML (on pourra se référer à 
 [l’appendice](046-asynchronous-javascript-and-xml-ajax.html#ajax) pour plus de détails).
...
Les différents paramètres de l’expérience sont précisés dans le
 tableau [5.1](#cap:Les-conditions-p=E9rimentales).
 |  |  |
| --- | --- |
...
La répartition des points haptiques sur la surface de travail de la souris
 ...
 zone d’intérêt. La figure [5.8](#cap:Exemples-des-conf)
 montre quelques configurations que les sujets ont dû explorer.



`![](img/hick1.svg)`
Figure 5.8 : Exemples des
...
La première donnée mesurable que nous pouvons commenter est le temps mis
 ...
 retour audio. Le tableau [5.2](#cap:Moyennes-des-temps) nous
 indique ces résultats, que la figure [5.9](#cap:Les-temps-selon)
 illustre.
...
![](img/res_loc_temps.png)
Figure 5.9 : Les temps selon
 les différentes conditions audios
...
À présent, la figure [5.10](#cap:Les-moyennes-tps) présente ces
 mêmes résultats, en les groupant selon le nombre de points présentés.



![](img/res_loc_temps_points.png)
Figure 5.10 : Les moyennes des
...
Au final, le tableau [5.3](#cap:Moyennes-des-notations,)
 présente la synthèse de ces notes, et la figure [5.11](#cap:Notations-selon-les) illustre le comportement des notes, selon
 la nature du retour audio.
 |  | Notes | Lettres | Silence | Moyenne |
| --- | --- | --- | --- | --- |
...
Table 5.3 : Moyennes des notations, sur 10,
...
![](img/res_loc_notes.png)
Figure 5.11 : Notations
 selon les différentes conditions audios
...
Maintenant, la figure [5.12](#cap:Notations,-selon-les)
 distingue les notes selon le nombre de points à découvrir.



![](img/res_loc_notes_points.png)
Figure 5.12 : Notations, selon les différentes
 conditions audios, et en fonction du nombre de points à trouver
...
Pour terminer, nous synthétisons tous ces résultats sur la figure [5.13](#cap:Corr=E9lation-entre-le).



![](img/res_correlation.png)
Figure 5.13 : Corrélation entre le temps mis lors
 de la tâche, et la notation. Les grands symboles représentent les
 positions moyennes pour chacune des conditions de retour audio
...
Hick et Hyman ([Hyman, 1953](047-bibliographie.html#Hyman1953)) on étudié
...
\begin{equation}
RT=a+b\log\_{2}(n)\label{eq:hick}\end{equation}
...
*b* est la pente ou le taux de traitement
 d’information et s’exprime en s/bit. Généralement, *b*=127−215 ms/bit d’information.


*a* est l’ordonnée à l’origine et est exprimé en
 seconde. On estime *a*=179 ms lorsque
 log2*n*=0,  *n*=1. Ainsi, *a* est la vitesse
...
Visuellement, la tâche peut être représentée comme indiqué sur la
 figure [5.14](#cap:Illustration-de-la). Cependant, lors de ce
...
Figure 5.14 : Illustration
 de la procédure, pour 4 points



Synthétiquement, les conditions expérimentales se trouvent dans le
 tableau [5.4](#cap:Param=E8tres-de-l'exp=E9rience). Ainsi, à
...
Afin de déduire le temps d’exploration *ET*,
 nous allons soustraire au temps total le temps de réaction *RT*, que nous obtenons grâce à la loi de Hick-Hyman, ainsi que
...
* 4 mouvements d’une longueur 3*A*/2
* et 2 mouvements d’une longueur 5*A*
...
* \(4a\) ms
* \(6\left[a+\log\_{2}\left(\frac{A}{2W}+1\right)\right]\) ms
* \(4\left[a+\log\_{2}\left(\frac{3A}{2W}+1\right)\right]\) ms
* \(2\left[a+\log\_{2}\left(\frac{5A}{2W}+1\right)\right]\) ms
...
Il nous reste alors à déduire la moyenne des temps, en fonction du nombre
 de zones. Les résultats des calculs sont dans le tableaux [5.5](#cap:Temps-th=E9oriques-des).
 |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| n | 2 | 3 | 4 | 6 | 9 |
| \(MT\_{moy}\) (en ms) | 499 | 528 | 550 | 586 | 627 |


Table 5.5 : Temps théoriques des mouvements
...
En s’appuyant sur la loi de Hick-Hyman, nous pouvons écrire les temps
 théoriques minimum et maximum de réaction, avec les coefficient *b*−=127 ms/bit., et *b*+=215 ms/bit.


* minimum :

 \begin{equation}
RT^{-}(n)=179+127\log\_{2}(n)\label{eq:RT1}\end{equation}
* et maximum :

 \begin{equation}
RT^{+}(n)=179+215\log\_{2}(n)\label{eq:RT2}\end{equation}


D’où l’évolution des temps théoriques minimum et maximum de réaction, en
 fonction du nombre de zones (tableau [5.6](#cap:Temps-de-r=E9action)).
 |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
...
Table 5.6 : Temps de réaction minimum et maximum
 théoriques
...
Les résultats de l’expérience sont présentés dans le tableau [5.7](#cap:R=E9sultats-de-l'exp=E9rience), et illustrés graphiquement sur
 la figure [5.16](#cap:=C9volution-du-temps).
 |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| Nombre de zones | 2 | 3 | 4 | 6 | 9 |
...
![](img/exp_explo.png)
Figure 5.16 : Évolution du temps de la tâche en
 fonction du nombre de zones



En partant de notre hypothèse de base, c’est à dire *Temps*=*ET*+*RT*+*MT*, nous pouvons déduire les
 valeurs minimum et maximum des temps d’exporation \(ET^{-}\) et \(ET^{+}\), en soustrayant aux temps de la
 tâche, les temps théoriques de réaction (\(ET^{-}\)et \(ET^{+}\) calculé avec les équations
 [3](#eq:RT1) et [4](#eq:RT2)), ainsi que les temps
 théoriques moyens des mouvements \(MT^{moy}\). Ce calcul est
 synthétisé dans le tableau [5.8](#cap:R=E9sultats-:-les).
...
Table 5.8 : Résultats : les temps moyens de la tâche
...
Traçons maintenant, la somme des temps de mouvement et d’exploration : on
 ...
 et déjà noter, sur la figure [5.17](#cap:Temps-d'exploration-et), que dans ce cas, le temps tombe à 0
 quand le nombre de zones est nul. Ceci tend à confirmer notre hypothèse de
 départ. En effet, une fois ôté le temps de réaction, le temps devrait se
 trouver nul, car il n’y a plus, ni exploration, et donc ni mouvement. À noter
 qu’avec une cible unique, le temps *MT*+*ET* est non nul. Cela provient
 du temps de déclenchement du geste (le terme *a*
 dans la loi de Fitts).



![](img/exp_explo2.png)
Figure 5.17 : Temps d’exploration et de mouvement,
 en fonction du nombre de zones



Soustrayons à présent le temps de mouvement *MT*, et traçons le résultat sur la figure [5.18](#cap:Temps-d'exploration-fx). Nous pouvons y observer que le temps
...
![](img/exp_explo3.png)
Figure 5.18 : Temps d’exploration en fonction du
 nombre de zones
...
D’après notre expérimentation, le temps d’exploration d’une surface, à la
 recherche d’une zone haptique, semble être linéaire en *n*. Cependant, notons bien que dans notre test, nous n’avons
...
## Application à la géographie : geogr’haptique
...
Le projet CDV (pour Cartographie pour Déficients Visuels [1](#note13)),
 au Canada ([Siekierska et al.,
 2001](047-bibliographie.html#Siekierska2001)), propose l’accès à des cartes prêtes à imprimer en se servant d’un
 amplificateur thermique : la représentation graphique (figure [5.19](#cap:Repr=E9sentation-graphique-d'une)) est téléchargée, imprimée
 sur du " papier-capsule " et passé dans un amplificateur thermique. Le
...
![](img/fo_fr_br.png)
Figure 5.19 : Représentation graphique
...
Enfin, le CDV, encore, propose une application des travaux de [(Gardner et Bulatov, 2001)](047-bibliographie.html#gardner). Il s’agit d’un
 environnement qui permet de lire des carte SVG relativement simples, et de
 les augmenter avec le mode haptique : sensation des frontières et des formes
 des régions.
...
Situation de départ : une carte des Etats-Unis est affichée dans le
 ...
 semble aimantée dans sa position actuelle (figure [5.20](#cap:geo1)).
...
Maintenant, on saisit la souris, et on la pousse vers la gauche. Il faut
 ...
 d’être prononcé par la synthèse vocale (figure [5.21](#cap:geo2)).
...
Tous nos testeurs ont donc souligné l’intérêt de l’approche, tout en
 ...
 cependant aux non-voyants) comme Google Maps[2](#note14), Yahoo Local[3](#note15), ou encore MSN
 Virtual Earth[4](#note16).
...
La théorie de l’harmonie de Longuet-Higgins ([Longuet-Higgins, 1962a](047-bibliographie.html#Longuet-Higgins1962);[Longuet-Higgins, 1962b](047-bibliographie.html#Longuet-Higgins1962a)) propose
 ...
 théorie de Longuet-Higgins ([Longuet-Higgins, 1962a](047-bibliographie.html#Longuet-Higgins1962);[Longuet-Higgins, 1962b](047-bibliographie.html#Longuet-Higgins1962a)) affirme que les
 intervalles qui interviennent dans la musique tonale occidentale sont ceux
 ...
 sont sur l’autre (figure [5.22](#cap:Repr=E9sentations-de-Longuet-Higgins)).


Plus tard, [(Steedman, 1972)](047-bibliographie.html#Steedman1972) a
 proposé une notation dérivée de celle de Longuet-Higgins et nous la
 détaillerons dans la prochaine section.



![](img/higinsSteedman.png)
Figure 5.22 : Représentations de
 Longuet-Higgins (à gauche) et de Steedman (à droite)
 ...
 la musique ([Howell et al., 1985](047-bibliographie.html#Howell1985)) en
 ...
 les travaux de Holland ([Holland,
 1987](047-bibliographie.html#Holland1987);[Holland, 1992](047-bibliographie.html#Holland1992)) sont basés sur
...
L’ordinateur a beaucoup été utilisé pour l’apprentissage de la musique. Et
 ...
 employées peuvent être très différentes. On pourra se référer à ([Brandao, 1999](047-bibliographie.html#Brandao1999)) pour une étude exhaustive des
...
* la visualisation graphique : ([Holland, 1987](047-bibliographie.html#Holland1987)) et ([Castaing, 2004](047-bibliographie.html#Castaing2004))
* l’intelligence artificielle : ([Holland, 1999](047-bibliographie.html#Holland1999))
...
Pour autant, ces approches oublient souvent les utilisateurs ayant une
 ...
 de la partition, en Braille. Les applications informatiques ([Braille Music Editor, 2002](047-bibliographie.html#bme)) et ([Toccata, 2002](047-bibliographie.html#Toccata)) fonctionnent de cette façon. Mais
...
Mark [(Steedman, 1972)](047-bibliographie.html#Steedman1972) a proposé une
 ...
 (figure [5.23](#cap:La-repr=E9sentation-de)).



![](img/Steedman.png)
Figure 5.23 : La représentation de Steedman et ses
...
Nous avons choisi cette notation, pour lui appliquer la localisation
 ...
 d’une note sont équidistants. Donc, la loi de [(Fitts, 1954)](047-bibliographie.html#fitts:54) nous donne un temps égal pour accéder
...
Les principales caractéristiques de notre représentation
 (figure [5.24](#cap:La-repr=E9sentation-stee)) sont :
...
`![](img/gamme.svg)`
Figure 5.24 : La représentation de Steedman,
 telle qu’elle est implé-mentée
...
La plus importante des caractéristiques de cette notation (et de celle de
 ...
 figure [5.25](#cap:Les-accord-impl=E9ment=E9s)) montre les
...
Autre avantage de cette notation, surtout dans une tâche
 ...
 la figure [5.26](#cap:Des-formes-diff=E9rentes), le même
...
![](img/trad1.png)
Figure 5.26 : Des formes différentes dans la
...
De plus, si deux accords semblent avoir la même forme dans la notation
 ...
 sur la figure [5.27](#cap:La-m=EAme-forme), les deux accords
...
![](img/trad2.png)
Figure 5.27 : La même forme dans la notation musicale,
...
D’un point de vue système, le fonctionnement de Music’haptic est
 schématisé sur la figure [5.28](#cap:Survol-du-syst=E8me).
...
* le mode damier : dans ce mode, l’utilisateur
 ...
 centre (figure [5.29](#cap:Une-note-et)). La note est jouée
 en même temps.
...
Le clavier permet de basculer entre les différents modes, ainsi que de
 ...
 etc. voir la figure [5.25](#cap:Les-accord-impl=E9ment=E9s)).
